...
async def fetch_polymarket_resolution(market_id: str, event_slug: Optional[str] = None) -> Tuple[bool, Optional[float]]:
    ...
    try:
        ...
        logger.warning(f"Fallo al obtener resolución para {event_slug or market_id}: {e}")
        return False, None
...
async def _search_market_in_events(market_id: str) -> Tuple[bool, Optional[float]]:
    ...
    try:
        ...
        logger.warning(f"Fallo al buscar el mercado {market_id}: {e}")
        return False, None
...
def _parse_market_resolution(market: dict) -> Tuple[bool, Optional[float]]:
    ...
    try:
        ...
        logger.warning(f"Fallo al analizar precios de los resultados: {e}")
        return False, None
...
async def _fetch_kalshi_resolution(ticker: str) -> Tuple[bool, Optional[float]]:
    ...
    try:
        ...
        logger.warning(f"Fallo al obtener resolución de Kalshi para {ticker}: {e}")
        return False, None
...
async def settle_pending_trades(db: Session) -> List[Trade]:
    ...
    try:
        ...
        logger.error(f"Fallo al consultar operaciones pendientes: {e}")
        return []
    ...
    for trade in pending:
        try:
            ...
            logger.error(f"Fallo al liquidar la operación {trade.id}: {e}")
            continue
    ...
    if settled_trades:
        try:
            ...
            logger.error(f"Fallo al confirmar liquidaciones: {e}")
            db.rollback()
            return []
    else:
...
async def update_bot_state_with_settlements(db: Session, settled_trades: List[Trade]) -> None:
    ...
    try:
        ...
        logger.error(f"Fallo al actualizar el estado del bot: {e}")
        db.rollback()
